...
export default function FloatingWhatsApp() {
  // International format for WhatsApp link. Assuming India (+91) based on standard usage for 10-digit numbers.
  const phoneNumber = '916207462800'; 
  const message = 'Hello! I need some help with my grocery order.';
  ...
  return (
    ...
    >
      <svg
        className="w-5 h-5 sm:w-6 sm:h-6 fill-current group-hover:scale-110 transition-transform duration-300"
        viewBox="0 0 24 24"
      >
      ...
      <span className="absolute -top-1 -right-1 flex h-3 w-3 sm:h-3.5 sm:w-3.5">
        ...
        <span className="relative inline-flex rounded-full h-3 w-3 sm:h-3.5 sm:w-3.5 bg-red-500 border border-white"></span>
      </span>
    </a>
  );
}
