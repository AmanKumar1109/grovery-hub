...
export default function SearchBar() {
  ...
  // Use real data from the database/context
  const trendingSearches = products?.length > 0 
    ? products.slice(4, 8).map(p => p.name) 
    : ["Fresh Milk", "Red Onions", "Whole Wheat Bread", "Organic Eggs"];
  ...
  return (
    <div ref={searchContainerRef} className="relative flex-1 max-w-xl mx-4 lg:mx-8 z-50">
      <div className={`flex items-center bg-[#f8f9fa] border rounded-full px-2 py-1 transition-all duration-300 ${isSearchFocused ? 'border-amber-400 ring-4 ring-amber-400/20 shadow-lg bg-white' : 'border-gray-200/80 shadow-inner hover:border-amber-300'}`}>
        <input
          type="text"
          placeholder="Search for groceries, vegetables, etc..."
        ...
        <div className="relative flex items-center hidden sm:flex">
          ...
          {isCatDropdownOpen && (
            <div className="absolute right-0 top-11 w-48 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
              {categories.map((cat) => (
                <button
                  key={cat}
                  onClick={() => {
          ...
          <div className="p-4 sm:p-5 border-b border-slate-50">
            ...
            <div className="flex flex-wrap gap-2">
              {trendingSearches.map((term, i) => (
                <button key={i} className="flex items-center gap-1.5 bg-slate-50 hover:bg-amber-50 border border-slate-200 hover:border-amber-300 text-slate-600 hover:text-amber-700 text-xs font-bold px-3 py-1.5 rounded-full transition-all cursor-pointer hover:shadow-sm">
          ...
          <div className="p-4 sm:p-5 bg-slate-50/50">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {popularProducts.map((prod, i) => (
                <div key={i} className="flex items-center gap-3 p-2 rounded-2xl hover:bg-white border border-transparent hover:border-slate-200 hover:shadow-sm transition-all cursor-pointer group">
